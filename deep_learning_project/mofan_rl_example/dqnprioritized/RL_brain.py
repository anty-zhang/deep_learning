...
class Memory(object):  # stored as ( s, a, r, s_ ) in SumTree
    ...
    def sample(self, n):
        # b_idx.shape(32, 1), b_memory.shape=(32, 6), ISWeights.shape=(32,1)
        ...
        for i in range(n):
            a, b = pri_seg * i, pri_seg * (i + 1)       # (a, b) 每个分桶的区间
            v = np.random.uniform(a, b)                 # 在(a, b) 分桶内随机选择一个数值
            idx, p, data = self.tree.get_leaf(v)    # idx=树的数据部分的索引值, p=树的idx部分td-error值, data=树的idx处的data值
            prob = p / self.tree.total_p            # prob=树的idx处的td-error/ 总的td-error
            # ISWeights[i, 0] = np.power(p/np.min(self.tree.tree[-self.tree.capacity:]), -self.beta)
